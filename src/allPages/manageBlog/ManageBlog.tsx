...
export default function ManageBlogs() {
    ...
    const fetchBlogs = async () => {
      ...
      try {
        ...
        const { data } = await axios.get(
          `/api/manage_blog/all_blog?${params.toString()}`
        );
  ...
  // For category filter dropdown, get distinct categories from current blogs + "All"
  // Optionally you can fetch categories separately from API
  const categories = [
    "All",
    ...new Set(blogs.map((blog) => blog.category_name)),
  ];
...
//         // If your API returns array directly, use:
//         setBlogs(Array.isArray(data) ? data : []);
//         // If API returns { blogs: [...] } object, use:
//         // setBlogs(data.blogs || []);
//       } catch (error) {
//         console.error("Error fetching blogs:", error);
//       } finally {
...
//   const categories = [
//     "All",
//     ...Array.from(new Set(blogs.map((blog) => blog.category_name))),
//   ];
...
//   // For pagination window (max 5 pages shown)
//   const maxVisiblePages = 5;
